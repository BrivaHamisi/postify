<div class="flex gap-3 mb-4 {{ $depth > 0 ? 'ml-' . ($depth * 8) : '' }}">
    ...
    <div class="flex-1">
        <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-gray-500 dark:text-[#B0AFA9]">
                {{ $comment->created_at->diffForHumans() }}
                @if ($comment->updated_at->gt($comment->created_at->addMinutes(1)))
                    <span class="italic text-gray-400 dark:text-[#B0AFA9]">(Edited {{ $comment->updated_at->diffForHumans() }})</span>
                @endif
            </span>
        </div>
            ...
            @auth
                ...
                @if (auth()->id() === $comment->user_id)
                    ...
                    <form action="{{ route('comments.destroy', $comment->id) }}" method="POST" class="inline">
                        ...
                        @method('DELETE')
                        <button type="submit"
                                class="text-sm text-gray-500 dark:text-[#B0AFA9] hover:text-red-600 dark:hover:text-red-400 transition-colors duration-200"
